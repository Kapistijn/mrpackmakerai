...
export interface ProjectSettings { minecraft_version: string; loader: LoaderType; loader_version?: string; name: string; description: string; theme: ThemeType; theme_custom?: string; difficulty: DifficultyType; performance_preference: PerformancePreference; minimum_mods?: number; maximum_mods?: number; minimum_downloads?: number }
...
export interface CompatibilityMetrics { minecraft_version: string; loader: string; loader_version?: string; java_version?: string; dependency_count: number; duplicate_count: number; missing_mod_count: number; missing_library_count: number; incompatible_count: number; client_only_count: number; server_only_count: number; deprecated_count: number; abandoned_count: number; outdated_count: number; security_issue_count: number; performance_score?: number; estimated_ram_mb?: number; estimated_vram_mb?: number; estimated_cpu_load_percent?: number; estimated_startup_seconds?: number; download_size_bytes: number; installed_size_bytes: number }
export interface CompatibilityReport { status: CompatStatus; mods: CompatCheckItem[]; dependencies: CompatCheckItem[]; conflicts: CompatCheckItem[]; warnings: string[]; missing_libraries: string[]; export_ready: boolean; errors: string[]; metrics?: CompatibilityMetrics }
export interface AISettingsPublic { provider: string; base_url: string; model: string; timeout_seconds: number; max_tokens: number; temperature: number; context_size: number; configured: boolean; api_key_configured: boolean }
...
export interface SettingsOverview { ai: AISettingsPublic; voice: VoiceSettingsPublic; minecraft: MinecraftSettingsPublic; sources: SourcesSettingsPublic; mod_sources: Record<string, boolean>; modrinth_key_configured: boolean; curseforge_key_configured: boolean; modrinth_key_masked: string; admin_locked: boolean }
export interface AISettingsUpdate { provider?: string; base_url?: string; model?: string; timeout_seconds?: number; max_tokens?: number; temperature?: number; context_size?: number; api_key?: string }
