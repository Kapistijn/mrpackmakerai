"""Regression coverage for the consolidated 2.5.6 startup implementation."""
...
def test_version_is_single_sourced_and_bumped():
 ...
 assert main_module.app.version=='2.5.6'

def test_health_route_reports_version():
 from app.api.routes import health
 assert health.__version__=='2.5.6'
...
def test_start_bat_no_longer_has_ampersand_crash_and_is_current():
 ...
 assert '2.5.6' in text

def test_start_ps1_uses_quote_safe_preflight_and_explicit_native_logging():
 ...
 assert 'RedirectStandardOutput' in ps1 and 'RedirectStandardError' in ps1
 assert "@('-c','import app.main; print(1)')" in ps1
 assert 'Backend launcher not found' in ps1
 assert 'between 1 and 65535' in ps1
 assert "$Version = '2.5.6'" in ps1
