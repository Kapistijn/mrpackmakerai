...
def mod(name,deps=()):return ModEntry(id=name,source='modrinth',name=name,slug=name,dependencies=list(deps),file_name=name+'.jar',file_size=10,download_url='https://cdn.modrinth.com/'+name+'.jar',hashes=ModHash(sha1='a'*40))
...
def test_legacy_unsafe_filename_contract_and_traversal_protection():
 codes=[x.code for x in validate_export_inputs(project(),[mod('../outside')])];assert 'unsafe_file_name' in codes;assert not is_safe_install_path('../outside.jar') and is_safe_install_path('config/a.json')
def test_dependency_metadata_reports_contextual_issues():
 a=mod('a',(ModDependency(project_id='b',dependency_type='required',loaders=['forge'],minecraft_versions=['1.19.2'],version_range='>=2'),));report=analyze_dependencies([a],minecraft_version='1.20.1',loader='fabric');assert {'missing','loader_conflict','minecraft_conflict'} <= {x['type'] for x in report['issues']};assert all({'cause','affected_mods','confidence','recommended_fixes'} <= set(x) for x in report['issues'])
