...
class AIOrchestrator:
    def __init__(self) -> None:
        self._active = {}; self._events = {}; self._final = {}
    ...
    async def generate(self, project_id: int, *, use_ai: bool = True) -> None:
        ...
        resolver = ModResolver(registry=registry); diagnostics = GenerationDiagnostics()
        try:
            async with AsyncSessionLocal() as db:
                ...
                diagnostics.requested = requirements.minimum_mods or requirements.maximum_mods
                ...
                if use_ai and provider is not None:
                    ...
                    await self._emit(project_id, AIProgressEvent(step=2, message="Using deterministic design plan...", data={"intent":intent.to_dict(),"categories":design.categories}), run)
                await self._emit(project_id, AIProgressEvent(step=3, message="Searching planned categories and expanded queries..."), run)
                candidates = await self._gather_candidates(registry, resolver, queries, mc_version, loader, requirements, seed=seed); diagnostics.found = len(candidates)
                ...
                await self._emit(project_id, AIProgressEvent(step=5, message="Resolving dependencies with safe graph limits..."), run)
                resolved_mods = []; resolved_keys = set(); pending = [(resolver.mod_key(item), 0) for item in selected]; guard = DependencyGraphGuard()
                while pending and len(resolved_mods) < max(500, target_count * 2):
                    ...
                    except DependencyGraphLimitError as exc:
                        raise RuntimeError(str(exc)) from exc
                    except UnknownModSourceError:
                        diagnostics.skip("unknown_source"); continue
                ...
                await self._emit(project_id, AIProgressEvent(step=6, message="Self-reviewing intent coverage, balance and compatibility...", data={"quality_score":quality.overall,"quality":quality.__dict__,"requirement_check":final_check.to_dict(),"profile":profile.as_pack_info(),"diagnostics":diagnostics.snapshot()}), run)
            ...
            logger.exception("Generation failed for project %d", project_id); await self._mark_failed(project_id, str(exc)); await self._emit(project_id, AIProgressEvent(step=0, message=f"Generation failed: {exc}", status="error", data={"diagnostics":diagnostics.snapshot()}))
        finally:
