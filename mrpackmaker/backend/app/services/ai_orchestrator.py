...
class AIOrchestrator:
    ...
    def _fallback_queries(project, prompt):
        ...
        queries = (
            ...
            + list(design.search_queries or ())
            + list(THEME_CATEGORIES.get(ThemeType(project.theme), ()))
            + POPULAR_FALLBACK_QUERIES
            + hints["prefer_queries"]
        )
        return [
            query
            for query in dict.fromkeys(queries)
            if not any(term in query.casefold() for term in hints["avoid_queries"])
        ]
    ...
    async def _gather_candidates(
        ...
        One page per query was enough for small packs but silently capped large
        generations at a few dozen useful candidates. We now walk a bounded
        number of pages and stop as soon as the global evidence pool is large
        enough. Providers retain their own shared rate limiter and cache.
        """
        del resolver
        candidates = {}
        ...
        for query in query_values:
            for source in registry.providers(available_only=True):
                for page in range(pages_per_query):
                    ...
                    try:
                        hits, total = await source.search(
                            query,
                            mc,
                            loader,
                            limit=page_size,
                        ...
                        logger.warning(
                            "Search failed on %s for '%s' page %d: %s",
                            source.source_id,
                            query,
                            page,
                            exc,
                    ...
                    for hit in hits:
                        text = " ".join(
                            (hit.name, hit.slug, hit.summary, *hit.categories)
                        )
                        if requirements is None or (
                            hit.downloads >= requirements.minimum_downloads
                            and theme_matches(text, requirements)
                        ):
        ...
        return [
            item.mod
            for item in rank_mods(list(candidates.values()), requirements, seed=seed)
        ]
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                intent = analyze_intent(
                    prompt, theme=project.theme, forbidden=req.forbidden_features
                )
                ...
                query_parts = [
                    self._fallback_queries(project, prompt),
                    list(req.required_features),
                    list(intent.categories),
                ]
                if not hints["low_hardware"]:
                    query_parts.append(shader_loader_queries(profile, loader.value))
                queries = list(
                    dict.fromkeys(item for part in query_parts for item in part)
                )
                target = max(
                    req.minimum_mods or 40,
                    min(
                        profile.max_content_mods,
                        req.maximum_mods or profile.max_content_mods,
                    ),
                )
                await self._emit(
                    project_id,
                    AIProgressEvent(
                        step=1,
                        message="Intent analysis and candidate search planning...",
                        data={
                            "intent": intent.to_dict(),
                            "profile": profile.as_pack_info(),
                ...
                if use_ai and provider:
                    try:
                        analysis = await provider.chat_json(
                            ...
                            user_prompt=brief.as_user_prompt(),
                        ...
                        if analysis.target_mod_count:
                            target = max(
                                req.minimum_mods or 1,
                                min(profile.max_content_mods, analysis.target_mod_count),
                            )
                        queries = list(
                            dict.fromkeys(
                                [
                                    *queries,
                                    *(analysis.gameplay_style or []),
                                    *(analysis.required_mods or []),
                                ]
                            )
                        )
                        try:
                            ai_intent = await provider.chat_json(
                                system_prompt=brief.system_prompt,
                                user_prompt=brief.as_user_prompt(),
                                schema=IntentAnalysisSchema,
                            )
                            intent = merge_ai_intent(
                                intent,
                                goal=ai_intent.goal or None,
                                categories=ai_intent.categories,
                                avoid=ai_intent.avoid,
                                realism_focus=ai_intent.realism_focus,
                            )
                            queries = list(
                                dict.fromkeys([*queries, *intent.categories])
                            )
                        except AIProviderError:
                            pass
                        gameplay = await provider.chat_json(
                            system_prompt=brief.system_prompt,
                            user_prompt=f"{brief.as_user_prompt()}\n{analysis.model_dump_json()}",
                        ...
                        queries = list(
                            dict.fromkeys([*plan.search_queries, *queries])
                        )
                ...
                selected = select_quality(
                    rank_by_quality(
                        ...
                        intent,
                        profile,
                        minimum_downloads=req.minimum_downloads,
                    ),
                    target,
                    profile,
                )
                if hints["low_hardware"]:
                    selected = [
                        ...
                        for mod in selected
                        if not any(
                            term in " ".join(mod.categories).casefold()
                            for term in ("worldgen", "dimension", "particle", "shader")
                        )
                    ] or selected[:1]
                initial_critique = critique_pack(
                    selected,
                    intent.categories,
                    ram_gb=project.target_ram_gb,
                    fps_target=project.target_fps,
                    shader_support=project.shader_support,
                )
                if initial_critique["missing_categories"]:
                    additions = fill_missing(
                        candidates,
                        selected,
                        verify_requirements(selected, intent),
                        intent,
                        profile,
                    )
                ...
                while pending and len(resolved) < max(500, target * 2):
                    ...
                    if (
                        not entry
                        or not entry.file_name
                        or not entry.download_url
                        or mod_identity(entry) in seen
                    ):
                        continue
                    ...
                    pending.extend(
                        (
                            ModEntry(
                                ...
                                source=dependency.source or entry.source,
                                name=dependency.project_id,
                            ),
                            depth + 1,
                        )
                        for dependency in entry.dependencies
                        if dependency.dependency_type in {"required", "embedded"}
                        and dependency.project_id
                    )
                resolved = resolver.deduplicate(resolved)
                if req.minimum_mods and len(resolved) < req.minimum_mods:
                    raise RuntimeError(
                        "Dependencies reduced the result below the requested minimum"
                    )
                loader_version = project.loader_version or await resolver.resolve_loader_version(
                    loader, mc
                )
                if not loader_version:
                    raise RuntimeError(f"No compatible {loader.value} loader version found")
                final_feedback = selection_feedback(
                    resolved,
                    candidates,
                    intent.categories,
                    ram_gb=project.target_ram_gb,
                ...
                for mod in resolved:
                    ...
                    memory.append(
                        build_mod_memory(
                            mod,
                            reason=final_feedback["reasons"].get(
                                key, "Added during dependency closure"
                            ),
                            confidence=final_feedback["confidence"].get(key, 55),
                            alternatives=final_feedback["alternatives"].get(key, []),
                            requested_roles=intent.categories,
                        )
                    )
                project.mods_json = json.dumps(
                    [mod.model_dump(mode="json") for mod in resolved]
                )
                ...
                await self._emit(
                    ...
                    AIProgressEvent(
                        step=7,
                        message=f"Generation complete: {len(resolved)} mods.",
                        status="complete",
                        data={
                            "mod_count": len(resolved),
                            "dependency_repair": repair_status,
                            "compatibility": compatibility_report.model_dump(mode="json"),
                            "hardware_hints": hints,
                            "candidate_count": len(candidates),
                            "selection_feedback": final_feedback,
                            "mod_memory": memory,
    ...
    def start_generation(self, project_id, *, use_ai=True):
        ...
        self._active[project_id] = asyncio.create_task(
            self.generate(project_id, use_ai=use_ai)
        )
