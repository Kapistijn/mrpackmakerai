...
class AIOrchestrator:
    def __init__(self) -> None:
        self._active: dict[int, asyncio.Task[None]] = {}; self._events: dict[int, asyncio.Queue[AIProgressEvent | None]] = {}; self._final: dict[int, AIProgressEvent] = {}
    @staticmethod
    def _fallback_queries(project: Project, prompt: str) -> list[str]:
        queries = list(THEME_CATEGORIES.get(ThemeType(project.theme), [])); words = [w for w in prompt.split() if len(w) > 4]
    ...
    async def _gather_candidates(self, registry, resolver, queries, mc_version, loader, requirements: Requirements | None = None, *, seed: int = 0) -> list[ModEntry]:
        ...
        for query in list(dict.fromkeys([*queries[:16], ""])):
    ...
    async def generate(self, project_id: int, *, use_ai: bool = True) -> None:
        queue = self._events[project_id]; provider = create_ai_provider() if use_ai else None
        registry = ModSourceRegistry([ModrinthClient(config.apis.modrinth_key), CurseForgeClient(config.apis.curseforge_key)]); resolver = ModResolver(registry=registry)
        try:
            async with AsyncSessionLocal() as db:
                ...
                requirements = parse_requirements(prompt, theme=project.theme, minimum_mods=project.minimum_mods, maximum_mods=project.maximum_mods, minimum_downloads=project.minimum_downloads)
                if requirements.warnings: raise RuntimeError("; ".join(requirements.warnings))
                seed = project.id ^ int(datetime.now(timezone.utc).timestamp()); brief = optimize_prompt(prompt, minecraft_version=mc_version, loader=loader.value, theme=project.theme, difficulty=project.difficulty, performance_preference=project.performance_preference)
                ...
                if use_ai and provider is not None:
                    ...
                    try:
                        analysis = await provider.chat_json(system_prompt=brief.system_prompt + "\nReturn only structured requirements. Never invent missing information.", user_prompt=brief.as_user_prompt(), schema=RequirementAnalysisSchema)
                        if analysis.missing_information:
                            questions = "; ".join(analysis.missing_information); await self._emit(project_id, AIProgressEvent(step=1, message=f"More information required: {questions}", status="error", data={"missing_information": analysis.missing_information, "confidence": analysis.confidence}), run); raise RuntimeError(f"Missing required information: {questions}")
                        await self._emit(project_id, AIProgressEvent(step=2, message="Personalizing categories...", data=analysis.model_dump(mode="json")), run)
                ...
                await self._emit(project_id, AIProgressEvent(step=3, message="Finding requirement-matched mods..."), run); candidates = await self._gather_candidates(registry, resolver, queries, mc_version, loader, requirements, seed=seed)
                if requirements.minimum_mods is not None and len(candidates) < requirements.minimum_mods: raise RuntimeError(f"Only {len(candidates)} compatible mods matched the requested minimum")
                if not candidates: raise RuntimeError("No compatible mods matched the selected requirements")
                await self._emit(project_id, AIProgressEvent(step=4, message="Scoring personalized candidates..."), run); selected = select_diverse(candidates, target_count)
                ...
                while pending and len(resolved_mods) < max(300, target_count * 2):
                    ...
                    if mod_identity(entry) in {mod_identity(item) for item in resolved_mods}: continue
                ...
                await self._emit(project_id, AIProgressEvent(step=6, message="Running final quality checks...", data={"mod_count": len(resolved_mods)}), run)
                ...
                project.mods_json = json.dumps([mod.model_dump(mode="json") for mod in resolved_mods]); project.resolved_loader_version = loader_version; project.ai_summary = f"Personalized {project.theme} pack: {len(resolved_mods)} requirement-matched mods."; project.status = ProjectStatus.REVIEW.value; run.status = "completed"; run.summary = project.ai_summary; run.completed_at = datetime.now(timezone.utc)
                await self._emit(project_id, AIProgressEvent(step=7, message=f"Generation complete: {len(resolved_mods)} mods.", status="complete", data={"mod_count": len(resolved_mods), "target_count": target_count, "seed": seed}), run); await db.commit()
