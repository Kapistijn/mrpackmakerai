 ...
 async def generate(self,project_id:int,*,use_ai=True)->None:
  ...
  try:
   async with AsyncSessionLocal() as db:
    ...
    intent=analyze_intent(prompt,theme=project.theme,forbidden=requirements.forbidden_features);profile=build_pack_profile(requirements);plan=build_discovery_plan(project.ai_creativity,project.discovery_depth)
    brief=optimize_prompt(prompt,minecraft_version=mc,loader=loader.value,theme=project.theme,difficulty=project.difficulty,performance_preference=project.performance_preference);design=build_pack_design(requirements)
    ...
    if use_ai and provider:
     try:
      ...
      await self._emit(project_id,AIProgressEvent(step=2,message='Understanding requirements...',data=analysis.model_dump(mode='json')),run)
      queries=list(dict.fromkeys(queries+analysis.gameplay_style+analysis.required_mods+analysis.forbidden_mods))
      ...
      intent=merge_ai_intent(intent,goal=ai_intent.goal,categories=ai_intent.categories,avoid=ai_intent.avoid,realism_focus=ai_intent.realism_focus);queries=list(dict.fromkeys(queries+list(intent.categories)))
      gameplay=await provider.chat_json(system_prompt=brief.system_prompt,user_prompt=f'{brief.as_user_prompt()}\nDesign: {design}\nAnalysis: {analysis.model_dump_json()}',schema=GameplayAnalysis)
      plan_ai=await provider.chat_json(system_prompt=brief.system_prompt,user_prompt=f'{brief.as_user_prompt()}\nDesign: {design}\nAnalysis: {gameplay.model_dump_json()}',schema=CategoryPlan);queries=list(dict.fromkeys(queries+plan_ai.search_queries))
    ...
    await self._emit(project_id,AIProgressEvent(step=3,message='Searching catalog with configured discovery depth...'),run)
    candidates=await self._gather_candidates(registry,queries,mc,loader,requirements,plan)
    required_missing=[item for item in requirements.constraint.required_mods if not any(mod_matches_constraint(m,item) for m in candidates)]
    ...
    await self._emit(project_id,AIProgressEvent(step=4,message='Scoring intent, quality and performance cost...'),run)
    target=max(requirements.minimum_mods or 40,min(profile.max_content_mods,requirements.maximum_mods or profile.max_content_mods));ranked=rank_by_quality(candidates,intent,profile,minimum_downloads=requirements.minimum_downloads);selected=select_quality(ranked,target,profile)
    ...
    if not check.complete:
     selected.extend(fill_missing(candidates,selected,check,intent,profile));check=verify_requirements(selected,intent)
    ...
    resolved=[];pending=[resolver.mod_key(m) for m in selected];seen=set()
    ...
    final_check=verify_requirements(resolved,intent);quality=review_pack(resolved,requirements)
    await self._emit(project_id,AIProgressEvent(step=6,message='Self-checking intent, assets and compatibility...',data={'quality_score':quality.overall,'requirement_check':final_check.to_dict(),'profile':profile.as_pack_info()}),run)
    ...
    project.mods_json=json.dumps([m.model_dump(mode='json') for m in resolved]);project.resolved_loader_version=loader_version;project.ai_summary=f'Designed {intent.goal}: {len(resolved)} mods, coverage {len(final_check.satisfied)}/{len(intent.categories)}';project.status=ProjectStatus.REVIEW.value;run.status='completed';run.summary=project.ai_summary;run.completed_at=datetime.now(timezone.utc)
    await self._emit(project_id,AIProgressEvent(step=7,message=f'Generation complete: {len(resolved)} mods.',status='complete',data={'mod_count':len(resolved),'requirement_check':final_check.to_dict()}),run);await db.commit()
