...
async def discover(registry,queries,project,limit=30):
 ...
 intent=analyze_intent(' '.join(queries),theme=project.theme);profile=build_pack_profile(req);found={};loader=LoaderType(project.loader)
 for query in dict.fromkeys(queries):
  for source in registry.providers(available_only=True):
   try:hits,_=await source.search(query,project.minecraft_version,loader,limit=limit)
