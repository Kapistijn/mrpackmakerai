...
class AsyncCoalescingCache:
 async def get_or_fetch(self,key,fetch):return await shared_catalog_cache.get_or_fetch(key,fetch)
class WorkerGenerationEngine:
 ...
 def merge_group(left:WorkerCandidate,right:WorkerCandidate)->WorkerCandidate:
  ...
  overlap=len(left.keyset()&right.keyset());union=max(1,len(left.keyset()|right.keyset()));redundancy=100.0*overlap/union;unique_bonus=min(10.0,max(0,len(merged)-overlap)*0.05);score=(left.score+right.score)/2+unique_bonus-(redundancy*0.03)
 ...
 @classmethod
 def merge_rounds(cls,candidates:list[WorkerCandidate]):
 ...
 async def _search(self,provider,query,mc,loader):return await self.cache.get_or_fetch(f'{provider.source_id}:{mc}:{loader.value}:{query.casefold()}',lambda:provider.search(query,mc,loader,limit=50,offset=0))
 async def _worker(self,directive,prompt,mc,loader,target):
  ...
  async def search_source(catalog,query):
   try:return await self._search(catalog,query,mc,loader)
   except Exception as exc:return ([],0)
  results=await asyncio.gather(*(search_source(catalog,query) for catalog in self.registry.providers(available_only=True) for query in queries[:8]))
...
class _WorkerPlan(BaseModel):queries:list[str]=[];rationale:str='';intent_coverage:float=50;performance:float=50
