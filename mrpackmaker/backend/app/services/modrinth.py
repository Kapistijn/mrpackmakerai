...
class ModrinthClient:
 ...
 async def _get(self,path:str,**kwargs:Any)->httpx.Response:
  return await modrinth_limiter.run(lambda:self._client.get(path,**kwargs))
 async def _coalesced(self,key:str,loader:Any)->Any:
  cached=detail_cache.get(key)
  if cached is not None:return cached
  task=self._inflight.get(key)
  if task is None:
   task=asyncio.create_task(loader()); self._inflight[key]=task
 ...
 async def search(self,query:str,mc_version:str,loader:LoaderType,category:str|None=None,limit:int=20,offset:int=0)->tuple[list[ModEntry],int]:
  ...
  try:
   ...
   if getattr(exc,'response',None) is not None and exc.response.status_code==429: logger.warning('Modrinth search rate-limited after retries; continuing with cached/other sources')
   else: logger.warning('Modrinth search failed: %s',exc)
   return [],0
 ...
 async def get_project(self,project_id:str)->dict[str,Any]|None:
  ...
  async def load():
   try:
    r=await self._get(f'/project/{project_id}');r.raise_for_status();data=r.json();detail_cache.set(key,data);return data
   except httpx.HTTPError as exc:
    if getattr(exc,'response',None) is not None and exc.response.status_code==429:logger.warning('Modrinth project %s unavailable after rate-limit retries',project_id)
    else:logger.warning('Modrinth get project failed: %s',exc)
    return None
 ...
 async def get_versions(self,project_id:str,mc_version:str,loader:LoaderType)->list[dict[str,Any]]:
  ...
  async def load():
   try:
    r=await self._get(f'/project/{project_id}/version',params={'loaders':json.dumps([loader.value]),'game_versions':json.dumps([mc_version])});r.raise_for_status();data=r.json();detail_cache.set(key,data);return data
   except httpx.HTTPError as exc:
    if getattr(exc,'response',None) is not None and exc.response.status_code==429:logger.warning('Modrinth versions for %s unavailable after rate-limit retries',project_id)
    else:logger.warning('Modrinth get versions failed: %s',exc)
    return []
  return await self._coalesced(key,load)
 async def get_version(self,version_id:str)->dict[str,Any]|None:
  ...
  async def load():
   try:
    r=await self._get(f'/version/{version_id}');r.raise_for_status();data=r.json();detail_cache.set(key,data);return data
   except httpx.HTTPError as exc:logger.warning('Modrinth get version failed: %s',exc);return None
  return await self._coalesced(key,load)
 async def get_mod_detail(self,project_id:str,mc_version:str,loader:LoaderType)->ModEntry|None:
  project=await self.get_project(project_id)
  if not project:return None
  versions=await self.get_versions(project_id,mc_version,loader);version=self.select_best_version(versions)
 ...
 async def search_loader_version(self,loader:LoaderType,mc_version:str)->str|None:
  ...
  if not project:return None
  versions=await self.get_versions(project,mc_version,loader);best=self.select_best_version(versions);return best.get('version_number') if best else None
