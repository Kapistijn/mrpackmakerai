...
def _safe_mod_filename(filename:str)->bool:return isinstance(filename,str) and bool(filename) and '/' not in filename and '\\' not in filename and filename not in {'.','..'} and PurePosixPath(filename).name==filename
