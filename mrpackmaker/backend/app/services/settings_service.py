...
class SettingsService:
 ...
 def set_model(self,model:str)->AISettingsPublic:
  public_config=self._read_public_config();ai_data=self._ai_data(public_config);ai_data['model']=model.strip();new_ai=AIConfig(**ai_data);public_config['ai']=new_ai.model_dump(exclude={'api_key'});self._write_public_config(public_config);config.ai=new_ai;return _public_ai()
 async def update_settings(self,update:UnifiedSettingsUpdate)->SettingsOverview:
  public_config=self._read_public_config();ai_data=self._ai_data(public_config);voice_data=self._voice_data(public_config);minecraft_data=dict(public_config.get('minecraft',{}));sources_data=dict(public_config.get('sources',{}));secret_set={};secret_clear=[]
  if update.ai is not None:
   for name in ('provider','base_url','model','timeout_seconds','max_tokens','temperature','top_p','retry_attempts','context_size'):
    ...
    if value is not None:ai_data[name]=value
   if update.ai.api_key is not None:
    key=update.ai.api_key.strip();ai_data['api_key']=key;(secret_set.__setitem__('ai_api_key',key) if key else secret_clear.append('ai_api_key'))
  if update.voice is not None:
   for name in ('whisper_url','tts_provider','tts_base_url','tts_model','tts_voice'):
    ...
    if value is not None:voice_data[name]=value
   if update.voice.tts_api_key is not None:
    key=update.voice.tts_api_key.strip();voice_data['tts_api_key']=key;(secret_set.__setitem__('tts_api_key',key) if key else secret_clear.append('tts_api_key'))
  if update.minecraft is not None:
   for name in ('default_version','default_loader'):
    value=getattr(update.minecraft,name)
    if value is not None:minecraft_data[name]=value
  if update.sources is not None:
   for name in ('modrinth_enabled','curseforge_enabled'):
    value=getattr(update.sources,name)
    if value is not None:sources_data[name]=value
  for field_name,store_key in (('modrinth_key','modrinth_key'),('curseforge_key','curseforge_key')):
   value=getattr(update,field_name)
   if value is not None:
    stripped=value.strip()
    if stripped:secret_set[store_key]=stripped
    else:secret_clear.append(store_key)
  new_ai=AIConfig(**ai_data);new_voice=VoiceConfig(**voice_data);new_minecraft=MinecraftConfig(**minecraft_data);new_sources=SourcesConfig(**sources_data);store=SecretStore(config.data_dir)
  if secret_set:store.update(secret_set)
  if secret_clear:store.remove(secret_clear)
  public_config['ai']=new_ai.model_dump(exclude={'api_key'});public_config['voice']=new_voice.model_dump(exclude={'tts_api_key'});public_config['minecraft']=new_minecraft.model_dump();public_config['sources']=new_sources.model_dump();public_config.setdefault('apis',{});public_config['apis'].pop('modrinth_key',None);public_config['apis'].pop('curseforge_key',None);self._write_public_config(public_config);config.ai=new_ai;config.voice=new_voice;config.minecraft=new_minecraft;config.sources=new_sources
 ...
 async def delete_secret(self,name:str)->SettingsOverview:
  ...
  if not store_key:raise KeyError(name)
  SecretStore(config.data_dir).remove([store_key])
  if store_key=='modrinth_key':config.apis.modrinth_key=''
  elif store_key=='curseforge_key':config.apis.curseforge_key=''
  elif store_key=='ai_api_key':config.ai.api_key=''
  elif store_key=='tts_api_key':config.voice.tts_api_key=''
  return await self.overview()
 ...
 def update(self,update:AdminSettingsUpdate)->AdminSettingsResponse:return self.admin_view()
 @staticmethod
 def _ai_data(public_config:dict)->dict:
  ...
  ai_data['api_key']=config.ai.api_key;return ai_data
 @staticmethod
 def _voice_data(public_config:dict)->dict:
  voice_data=dict(public_config.get('voice',{}));voice_data['tts_api_key']=config.voice.tts_api_key;return voice_data
 @staticmethod
 ...
 def _write_public_config(contents:dict)->None:
  ...
  with NamedTemporaryFile('w',encoding='utf-8',delete=False,dir=path.parent) as temporary:json.dump(contents,temporary,indent=2);temporary.write('\n');temporary_path=Path(temporary.name)
