...
async def propose(project,prompt):
 current=[ModEntry.model_validate(x) for x in __import__('json').loads(project.mods_json or '[]')]
 plan=plan_change(prompt,current);registry=create_default_registry()
 try:
...
async def apply(project,prompt,approved_plan,db):
 current=[ModEntry.model_validate(x) for x in __import__('json').loads(project.mods_json or '[]')]
 remove=set(approved_plan.get('remove_names',[])); kept=[m for m in current if m.name not in remove]
 registry=create_default_registry()
 try:
  candidates=await discover(registry,approved_plan.get('add_queries',[]),project)
  existing={mod_identity(m) for m in kept}; additions=[m for m in candidates if mod_identity(m) not in existing][:10]
  resolver=ModResolver(registry=registry); result=await DependencyResolver(resolver).resolve_pack(kept+additions,project.minecraft_version,project.loader)
  if not result.complete: raise ValueError('; '.join(f.message() for f in result.failures))
  project.mods_json=__import__('json').dumps([m.model_dump(mode='json') for m in result.mods]);project.mrpack_path=None
