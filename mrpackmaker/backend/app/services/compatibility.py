...
class CompatibilityService:
 ...
 async def check_project(self,project:Project)->CompatibilityReport:
  ...
  for mod in selected:
   ...
   local_import=fresh is None and mod.source=='imported' and bool(mod.file_name and mod.download_url and (mod.hashes.sha1 or mod.hashes.sha512))
   effective=fresh or mod
   if (fresh and fresh.file_name and fresh.download_url) or local_import:
    resolved.append(effective);graph.add_mod(effective);mod_items.append(CompatCheckItem(name=effective.name,status=CompatStatus.OK,message='Imported artifact metadata accepted' if local_import else 'Version and loader match'))
   else:
    errors.append(f'{mod.name} has no compatible {project.minecraft_version} {loader.value} file');mod_items.append(CompatCheckItem(name=mod.name,status=CompatStatus.ERROR,message='No compatible file'))
  identities=[mod_identity(mod) for mod in selected];duplicate_count=len(identities)-len(set(identities))
