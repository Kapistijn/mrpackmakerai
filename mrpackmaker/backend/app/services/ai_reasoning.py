...
Catalog facts are gathered first, then scored, checked for missing intent
coverage, critiqued, and handed to the existing generation/export pipeline.
"""
from __future__ import annotations
from dataclasses import dataclass,asdict
from typing import Any
from app.schemas.mod import ModEntry
from app.services.pack_intelligence import quality_report,synergy_report,performance_estimate

@dataclass(frozen=True)
class ModMemory:
 name:str; source:str; categories:tuple[str,...]; roles:tuple[str,...]; dependencies:tuple[str,...]; downloads:int; performance_impact:str; compatibility:str; reason:str; confidence:int; alternatives:tuple[str,...]=()

_ROLE_SIGNALS=(
 (('create','automation','factory','technology','engineering','machine'), 'automation'),
 (('storage','inventory','backpack'), 'storage'),
 (('transport','rail','aircraft','vehicle'), 'transport'),
 (('food','farm','farming','agriculture','crop'), 'farming'),
 (('worldgen','world generation','terrain','biome','dimension'), 'world generation'),
 (('quest','progression','advancement'), 'progression'),
 (('magic','spell','arcane'), 'magic'),
 (('boss','combat','mob','monster'), 'combat'),
)
def _roles(mod:ModEntry)->tuple[str,...]:
 ...
 for signals,role in _ROLE_SIGNALS:
  if any(signal in text for signal in signals) and role not in roles:roles.append(role)
...
def build_mod_memory(mod:ModEntry,reason:str='',confidence:int=70,alternatives:tuple[str,...]=())->dict[str,Any]:
 impact='high' if any(x in ' '.join((mod.name,*mod.categories)).casefold() for x in ('worldgen','particle','shader','dimension')) else 'low'
...
def critique_pack(mods:list[ModEntry],requested:list[str],*,ram_gb=None,fps_target=None,shader_support=None)->dict[str,Any]:
 quality=quality_report(mods);synergy=synergy_report(mods);performance=performance_estimate(mods,ram_gb=ram_gb,fps_target=fps_target,shader_support=shader_support);missing=missing_categories(mods,requested);redundant=[];by_role={}
 for mod in mods:
 ...
 problems=[*({'type':'missing_category','category':item,'severity':'medium','recommendation':f'Search for more {item} content'} for item in missing),*({'type':'worldgen_overlap','mods':item['mods'],'severity':'high','recommendation':'Review configs before export'} for item in synergy['conflicts'])]
