...
class MrpackGenerator:
    def build_index(self, project: Project, mods: list[ModEntry]) -> dict[str, Any]:
        ...
        # A pinned version is a user decision and must win over the resolver's
        # latest-version fallback. This fixes the old UI gap where selection
        # looked possible but export silently used another runtime.
        selected_loader = project.loader_version or project.resolved_loader_version
        index: dict[str, Any] = {"formatVersion": 1, "game": "minecraft", "versionId": datetime.now(timezone.utc).strftime("%Y.%m.%d-%H%M%S"), "name": project.name, "summary": project.description, "files": [], "dependencies": {"minecraft": project.minecraft_version, loader_key: selected_loader}}
        for mod in mods:
            hashes = {algorithm: value for algorithm, value in {"sha1": mod.hashes.sha1, "sha512": mod.hashes.sha512}.items() if value}
            index["files"].append({"path": f"mods/{mod.file_name}", "hashes": hashes, "downloads": [mod.download_url], "fileSize": mod.file_size})
        return index

    def _validate_archive(self, path: Path) -> None:
        with zipfile.ZipFile(path, "r") as archive:
            corrupt_file = archive.testzip()
            if corrupt_file: raise RuntimeError(f"Corrupt ZIP member: {corrupt_file}")
            ...
            if not index.get("dependencies", {}).get("minecraft"): raise RuntimeError("MRPack is missing a Minecraft dependency")
            for file_entry in index.get("files", []):
                if not file_entry.get("hashes") or not file_entry.get("downloads"): raise RuntimeError("MRPack contains an unresolved file")
    ...
    def generate(self, project: Project) -> Path:
        ...
        descriptor, temporary_name = tempfile.mkstemp(prefix=f".{_sanitize_filename(project.name)}-", suffix=".mrpack", dir=config.output_dir)
        os.close(descriptor)
        temporary_path = Path(temporary_name)
        try:
            with zipfile.ZipFile(temporary_path, "w", zipfile.ZIP_DEFLATED) as archive: archive.writestr("modrinth.index.json", json.dumps(index, indent=2))
            self._validate_archive(temporary_path)
            temporary_path.replace(output_path)
        finally:
            if temporary_path.exists(): temporary_path.unlink(missing_ok=True)
