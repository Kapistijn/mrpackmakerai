...
from app.services.intent_taxonomy import IntentCategory,matches_category
...
class ModQualityScore:
 ...
 def total(self)->float:
  return round(.35*self.intent_match+.20*self.realism_score+.15*self.compatibility_score+.10*self.maintenance_score+.10*self.popularity+.10*(1-self.performance_cost)+.05*self.dependency_quality,5)
...
def is_blocked(score,profile):
 if score.is_performance_mod and not profile.needs_performance_mods:return True
 if score.performance_cost>=.8 and (profile.performance_floor>=.7 or not profile.allow_heavy_mods):return True
 return False
