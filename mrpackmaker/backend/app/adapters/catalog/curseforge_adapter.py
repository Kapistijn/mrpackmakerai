...
class CurseForgeAdapter:
    """Translate the existing CurseForgeClient DTOs into domain candidates."""

    def __init__(self, client: CurseForgeClient, minecraft_version: str, loader: Loader) -> None:
        self._client = client
    ...
    def _loader_type(loader: Loader) -> LoaderType:
        try:
            ...
            raise InvalidResponseError(f"Unsupported CurseForge loader: {loader.value}") from exc

    @staticmethod
    def _file(entry: ModEntry) -> ModFile:
        if not entry.file_name or not entry.download_url or not entry.hashes.sha512:
            raise InvalidResponseError("CurseForge response is missing file name, URL, or sha512")
        return ModFile(
    ...
    def _candidate(entry: ModEntry, *, require_file: bool = False) -> ModCandidate:
        ...
        files = (CurseForgeAdapter._file(entry),) if require_file or entry.file_name else ()
        identity = CanonicalModIdentity(
            canonical_key=f"curseforge:{entry.id}",
            display_name=entry.name,
            sources={ModSource.CURSEFORGE: entry.id},
