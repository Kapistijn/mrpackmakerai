...
async def apply_change(project_id:int,body:ApproveChange,db:AsyncSession=Depends(get_db)):
 ...
 if not request or request.project_id!=project_id:raise HTTPException(404,'Change plan not found')
 if request.status=='applied':raise HTTPException(409,'Change plan already applied')
 plan=json.loads(request.plan_json);mods,added,removed=await apply(project,request.prompt,plan,db)
 change=ModpackChange(project_id=project_id,action=plan.get('action','change'),mods_added=json.dumps([m.model_dump(mode='json') for m in added]),mods_removed=json.dumps([m.model_dump(mode='json') for m in removed]),reason=plan.get('reason',''),ai_prompt=request.prompt,impact=json.dumps({'mod_count':len(mods)}));db.add(change);request.status='applied';await db.flush();return {'change_id':change.id,'mods':len(mods),'status':'applied'}
