...
def _project_to_response(project: Project) -> ProjectResponse:
    ...
    return ProjectResponse(
        ...
        generation_prompt=project.generation_prompt, status=project.status_enum(),
        mods=mods, resolved_loader_version=project.resolved_loader_version,
...
async def loader_versions(
    mc: str = Query(..., min_length=3, max_length=32),
    loader: LoaderType = Query(...),
):
    """Return stable compatible loader versions for the project wizard."""
    client = ModrinthClient(config.apis.modrinth_key)
    try:
        versions = await client.get_versions(
            {LoaderType.FABRIC: "fabric-loader", LoaderType.FORGE: "forge", LoaderType.NEOFORGE: "neoforge"}[loader],
            mc,
            loader,
        )
        return {
            ...
            "loader": loader.value,
            "versions": [
                {"id": item.get("id"), "version": item.get("version_number"),
                 "type": item.get("version_type", "release"),
                 "published": item.get("date_published")}
                for item in versions if item.get("id") and item.get("version_number")
            ],
        }
    finally:
        await client.close()
...
async def list_projects(db: AsyncSession = Depends(get_db)):
    ...
    return [ProjectListItem(id=p.id, name=p.name, minecraft_version=p.minecraft_version,
                            loader=p.loader_enum(), status=p.status_enum(),
...
async def update_project(project_id: int, body: ProjectUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if body.loader_version is not None:
        project.loader_version = body.loader_version.strip() or None
        project.resolved_loader_version = None
        project.mrpack_path = None
...
async def delete_project(project_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if project.mrpack_path:
        ...
        if output_dir in path.parents and path.exists():
