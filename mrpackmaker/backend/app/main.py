...
from app.api.routes import ai,compatibility,health,modpack,mods,projects,settings,editor,imports,repair
...
app=FastAPI(title='MrPackMaker',description='AI Minecraft Modpack Generator',version='2.1.0',lifespan=lifespan)
...
app.include_router(health.router,prefix='/api',tags=['health']);app.include_router(projects.router,prefix='/api/projects',tags=['projects']);app.include_router(mods.router,prefix='/api/mods',tags=['mods']);app.include_router(ai.router,prefix='/api/ai',tags=['ai']);app.include_router(compatibility.router,prefix='/api/compatibility',tags=['compatibility']);app.include_router(modpack.router,prefix='/api/modpack',tags=['modpack']);app.include_router(settings.router,prefix='/api/settings',tags=['settings']);app.include_router(editor.router,prefix='/api/editor',tags=['editor']);app.include_router(imports.router,prefix='/api/imports',tags=['imports']);app.include_router(repair.router,prefix='/api/repair',tags=['repair'])
